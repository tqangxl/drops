(function () {
	...
	if (window["WebSocket"]) { 
		...
		conn.onmessage = function(evt) {
			...
			Drops.patch(JSON.parse(evt.data))
			Drops.route(JSON.parse(evt.data));
	...
	window.addEventListener('popstate', function(event) {
		Drops.loadContent(window.location.pathname);
	});	

	var Drops = window.Drops = {}  

	//Loads content from server using either websockets or http reload
	Drops.loadContent = function(url) {
		console.log('Loading content for: ' +url);
		if (typeof conn !== 'undefined' && conn != null && conn.readyState === 1) {
			conn.send(JSON.stringify({"type":"GET", "data": {"route":url,"params": {}}}));
		} else {
			window.location.reload();
		}
	}

	Drops.sendMessage = function(message) {
		if (typeof conn !== 'undefined' && conn != null && conn.readyState === 1) {
			conn.send(message);
		} else {
	...
	Drops.patch = function(message) {

    // var elements = document.getElementsByTagName(patch.element);
    // for (var i = 0, j = elements.length; i < j; i++) {
    // 	elements[i].innerHTML = patch.payload;
    // }
    // var element = document.getElementById(patch.element);
    // element.innerHTML = patch.payload;
    if (message != null && typeof message.patches != 'undefined' && message.patches != null) {
    	var patch = message.patches;
    	if (patch instanceof Array) {

    		for (var i = 0, j = patch.length; i < j; i++) {

    			var p = patch[i];
    			console.log('Patching for element: ' + p.element)
    			if (p.element === 'body' || p.element === '') {
    				window.location.reload();
    			} else {
    				$(p.element).html(p.payload);
    			}
    		}
    	} else {
    		$(patch.element).html(patch.payload);
    	}
    }
}

Drops.route = function(message) {
	if (message != null && typeof message.route != 'undefined' && message.route != null && message.route !== '') {
		var generatedUrl =  message.route
		window.history.pushState("", "", generatedUrl);
		Drops.loadContent(generatedUrl);
